...
import products.templatetags.cart
from grocerystore.settings import RAZORPAY_API_KEY, RAZORPAY_API_SECRET_KEY
        ...
        if len(phone) == 10:
            if phone[0] in "7896":
                for product in products:
                    order: Order = Order(user=User(id=user_id), product=product, price=product.price,
...
def payment(request):
   ...
   payment_order=client.order.create({'amount':order_amount,'currency':order_currency})
